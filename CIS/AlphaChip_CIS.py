...
i_test_image_count = 0
i_read_image_number = 0
b_illuminance = False
i_x_Mode = -1
b_Change_Image_addr = False
s_image_addr = ''
A_image_list = []

def CIS_ADC():
    global g_F_Process_image, i_x_Mode, s_image_addr, A_image_list, b_Change_Image_addr, i_read_image_number, b_illuminance, i_test_image_count
    
    if AlphaChip_Memory.g_b_Camera:
        if AlphaChip_Memory.g_RAW_CAM :
        ...
        if i_Target == AlphaChip_Memory.g_N_TARGET['PIRA_1']:
            if AlphaChip_ISP.g_i_Mode == AlphaChip_Memory.g_N_mode['IDLE_MODE']:
                ...
                if i_test_image_count > 20:
                    ...
                    b_illuminance = True
                elif i_test_image_count > 25:
                    s_image_type = 'Occupancy'  # HIGH
                    b_illuminance = True
                elif i_test_image_count > 125:
                    s_image_type = 'InterMove'  # HIGH
                    b_illuminance = True
                elif i_test_image_count > 130:
                    s_image_type = 'Background' # HIGH
                    b_illuminance = True
                elif i_test_image_count > 150:
                    s_image_type = 'Background' # LOW
                    b_illuminance = False
        ...
        s_image_addr = s_image_addr + s_image_type + '/Process/' + s_cis_mode_type + '_' + s_FPS_type + 'F'
        ...
        for dir_member in os.listdir(s_image_addr):                 #폴더 안에 파일 확인
            if (s_image_addr+'/'+dir_member).endswith('.pgm'):      #찾는 파일이 맞을경우
                ...
                if s_split_file_name[0] == s_scale_type and s_split_file_name[3] == i_read_image_number:
                    # if s_split_file_name[3] == i_read_image_number:
                    s_image_addr = s_image_addr+'/'+dir_member
                    print("s_image_addr : ", s_image_addr)
                    break

        g_F_Process_image = cv2.imread(s_image_addr, cv2.IMREAD_UNCHANGED)
        print("g_F_Process_image ; ", g_F_Process_image)
        i_test_image_count += 1
